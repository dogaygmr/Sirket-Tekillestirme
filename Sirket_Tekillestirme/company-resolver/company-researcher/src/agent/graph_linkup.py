...
class Affiliations(BaseModel):
    ...
    children: list[str] = Field(default=[], description="List of subsidiary or child companies. Leave the list empty ([]) if you can't find a subsidiary or child company.")
    partners: list[str] = Field(default=[], description="List of companies in active (not past) strategic partnerships or long-term collaborations. Leave the list empty ([]) if you can't find an active (not past) strategic partnership or long-term collaboration.")
...
class CompanyInfo(BaseModel):
    ...
    funding_summary: Optional[str] = Field(
        description="Summary of the company's funding history. Leave it null if the information is not available."
    )
...
async def research_company(
    ...
    async def async_search(query: str):
        """Wrapper to make LinkUp search async"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, linkup_retriever.invoke, query)

    # Search tasks
    search_tasks = []
    for query in state.search_queries:
        search_tasks.append(async_search(query))

    # Execute all searches concurrently
    search_results = await asyncio.gather(*search_tasks)

    # Transform LinkUp results to match expected format
    transformed_results = []
    for result_list in search_results:
        for doc in result_list:
            transformed_results.append({
                "results": [{
                    "title": doc.metadata.get("name", "No Title"),
                    "url": doc.metadata.get("url", "No URL"),
                    "content": doc.page_content,
                    "raw_content": None  # LinkUp doesn't provide this
                }]
            })
